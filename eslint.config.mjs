...
export default [
	...
	...compat.extends("eslint:recommended", "plugin:eslint-comments/recommended"),
	{
		...
		rules: {
			...
			"unused-imports/no-unused-vars": [
				"warn",
				{ args: "after-used", argsIgnorePattern: "^_", varsIgnorePattern: "^_\\d*" },
			],
		},
	},
		...
		})),
	{
		...
		rules: {
			...
			"@typescript-eslint/no-use-before-define": [
				"error",
				{ classes: false, functions: false, ignoreTypeReferences: true, typedefs: true },
			],
	...
	{
		files: ["jest.config.ts", "src/configs/*.ts", "src/index.ts", "eslint.config.mjs"],
	...
	...compat.extends("plugin:jsonc/base", "plugin:jsonc/recommended-with-json").map(config => ({
		...config,
		files: ["**/*.json"],
	})),
	{
		files: ["**/*.json"],
		rules: {
			"jsonc/array-bracket-newline": ["off", { minItems: 2, multiline: true }],
		...
		rules: {
			...
			"jsonc/sort-array-values": [
				"warn",
				{
					order: { type: "asc" },
					pathPattern:
						"compilerOptions.(lib|types)$|include$|files$|^exclude$|extends$|plugins$|cSpell.words|eslint.validate|auto-close-tag.\\w+|stylelint.\\w+|vscode.extensions$",
				},
			],
		},
	},
	...compat
		.extends("plugin:yml/prettier", "plugin:yml/recommended", "plugin:yml/standard")
		.map(config => ({
			...config,
			files: ["**/*.yaml", "**/*.yml"],
		})),
	{
		...
		rules: {
			...
			"yml/sort-sequence-values": ["warn", { order: { type: "asc" }, pathPattern: ".*" }],
		},
	},
	{
		files: ["**/*.sql"],
	...
	})),
	{
		...
		rules: {
			"markdownlint/md013": ["error", { line_length: 200 }],
			"markdownlint/md033": ["error", { allowed_elements: ["a", "img"] }],
	...
	{
		files: ["docs/rules/*.md"],
		rules: { "markdownlint/md024": "off", "markdownlint/md033": "off" },
	},
	{
		files: ["**/Dockerfile"],
		languageOptions: { parser: ymlParser },
